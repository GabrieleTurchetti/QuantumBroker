...
# Lowcost policy
def lowcost(dispatches, total_shots, level): # cost aware
...
# Fast policy
def fast(dispatches, total_shots, level): # time aware
    percentage = 100 - level
    new_size = math.ceil((len(dispatches) * percentage) / 100)
...
# Accurate policy
def accurate(dispatches, total_shots, level): # to change
    percentage = 100 - level
    new_size = math.ceil((len(dispatches)* percentage) / 100)
...
def fair(dispatches, total_shots, level): # uniform
    percentage = 100 - level
    new_size = math.ceil((len(dispatches) * percentage) / 100)
