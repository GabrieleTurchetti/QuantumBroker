...
def get_dispatch_value(dispatch, total_shots, metrics, min_values, max_values):
    ...
    normalized_total_energy_cost = (total_cost - min_values["total_energy_cost"]) / ((max_values["total_energy_cost"] - min_values["total_energy_cost"]) if max_values["total_energy_cost"] != min_values["total_energy_cost"] else 1)
    ...
    value = normalized_total_cost * (metrics["total_cost"] if "total_cost" in metrics else 0) + normalized_total_energy_cost * (metrics["total_energy_cost"] if "total_energy_cost" in metrics else 0) + normalized_total_time * (metrics["total_time"] if "total_time" in metrics else 0) + normalized_used_computers * (metrics["used_computers"] if "used_computers" in metrics else 0) + normalized_shots_difference * (metrics["shots_difference"] if "shots_difference" in metrics else 0)
